...
class ProblemPageExtender {
  ...
  async collapse() {
    ...
    $(window).scrollTop(0);
    $('.main > .row').not('.contest-background').show();
    $('.footer').show();
...
const page = new NamedPage(['problem_detail', 'contest_detail_problem', 'homework_detail_problem'], async () => {
  ...
  await confettiManager.loadConfig();
  ...
  async function loadReact() {
    ...
    sock.onmessage = (message) => {
      ...
      // Confetti插件集成 - 使用插件的confetti管理器
      if ('contest' in msg.rdoc) { /* empty */ } else {
        if (msg.rdoc.status === 1 && msg.rdoc.contest !== '000000000000000000000000') {
          store.dispatch({
            type: 'SCRATCHPAD_SET_POST_COUNT',
            payload: 1,
          });
