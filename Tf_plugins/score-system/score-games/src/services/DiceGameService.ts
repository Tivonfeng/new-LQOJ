...
export class DiceGameService {
    ...
    private gameLimitService: DailyGameLimitService;
    private config: any;
    ...
    constructor(config: any, ctx: Context, scoreService: IScoreService, gameLimitService: DailyGameLimitService) {
        this.config = config;
        this.ctx = ctx;
        this.scoreService = scoreService;
        this.gameLimitService = gameLimitService;
    }
    ...
    async playDiceGame(domainId: string, uid: number, guess: 'big' | 'small', betAmount: number = 20): Promise<{
        ...
        // 扣除投注积分
        console.log(`[DiceGame] Calling deductUserScore for user ${uid}, amount: ${betAmount}`);
        ...
        // 如果获胜，发放奖励
        if (won) {
            const rewardResult = await this.scoreService.addUserScore(
                domainId,
                uid,
                reward,
