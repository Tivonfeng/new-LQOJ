...
export class CheckInService {
    ...
    constructor(config: any, ctx: Context, scoreService: IScoreService) {
        this.config = config;
        this.ctx = ctx;
    ...
    async checkIn(domainId: string, uid: number): Promise<CheckInResult> {
        ...
        try {
            ...
            // 使用新的积分服务接口增加积分（会自动添加积分记录）
            const result = await this.scoreService.addUserScore(domainId, uid, score, `每日签到获得积分 (连续${newStreak}天)`);

            if (!result.success) {
                throw new Error(`积分添加失败: ${result.message}`);
            }
    ...
    calculateCheckInScore(streak: number): number {
        const baseScore = this.config.dailyReward || 5; // 基础签到奖励

        // 如果未启用连续签到奖励，直接返回基础分数
        if (!this.config.consecutiveBonus) {
            return baseScore;
        }

        if (streak === 1) return baseScore; // 第1天
        if (streak <= 3) return baseScore + 2; // 第2-3天
        if (streak <= 6) return baseScore + 3; // 第4-6天
        if (streak === 7) return baseScore + 10; // 第7天 (周奖励)
        if (streak <= 13) return baseScore + 5; // 第8-13天
        if (streak === 14) return baseScore + 20; // 第14天 (双周奖励)

        // 第15天及以后，但不超过最大奖励倍数
        const maxBonus = Math.min(8, (this.config.maxConsecutiveBonus || 10) - 1);
